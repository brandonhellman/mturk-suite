...
function todaysOverviewDisplay() {
  ...
  for (let i = sorted.length - 1; i > -1; i--) {
    ...
    requesterLink.href = `https://workersandbox.mturk.com/requesters/${req.id}/projects`;
...
function fetchQueue(date) {
  ...
  return new Promise((resolve) => {
    (async function fetchLoop() {
      try {
        const response = await fetch(`https://workersandbox.mturk.com/tasks?format=json`, {
          credentials: `include`,
        });

        if (response.ok && response.url === `https://workersandbox.mturk.com/tasks?format=json`) {
          const json = await response.json();
          resolve(json);
        } else if (response.url.indexOf(`https://workersandbox.mturk.com/`) === -1) {
...
function fetchDashboard(date) {
  ...
  return new Promise(async (resolve) => {
    (async function fetchLoop() {
      try {
        const response = await fetch(`https://workersandbox.mturk.com/dashboard?format=json`, {
          credentials: `include`,
        });

        if (response.ok && response.url === `https://workersandbox.mturk.com/dashboard?format=json`) {
          const json = await response.json();
          resolve(json);
        } else if (response.url.indexOf(`https://workersandbox.mturk.com/`) === -1) {
          return loggedOut();
    ...
    index.openCursor(only).onsuccess = (event) => {
      ...
      if (cursor) {
        if (
          ...
          (cursor.value.state === `Assigned` && !hit_ids.includes(cursor.value.hit_id))
        ) {
...
function sync(date) {
  return new Promise(async (resolve, reject) => {
    ...
    (async function fetchLoop(page) {
      const url = `https://workersandbox.mturk.com/status_details/${fetchDate}?page_number=${page}&format=json`;
      ...
      if (response.ok && response.url === url) {
        ...
        if (json.num_results > 0) {
          syncingUpdated(
            date,
            `Updating page ${page} of ${Math.ceil(json.total_num_results / 20)} for ${json.total_num_results} HITs`,
          );
...
async function dailyOverview() {
  ...
  if (dateTo || dateFrom) {
    ...
    objectStore.openCursor(
      IDBKeyRange.bound(dateFrom.replace(/-/g, ``) || `0`, dateTo.replace(/-/g, ``) || `99999999`),
    ).onsuccess = (event) => {
      const cursor = event.target.result;
