...
(async () => {
  const items = await new Promise((resolve) =>
    chrome.storage.local.get([`hitCatcher`, `order`, `watchers`, `hitCatcherWatchers`], resolve),
  );
...
function addWatcher() {
  bootbox.prompt({
    ...
    callback(result) {
      if (result) {
        const watcher = {
          id: result.match(/projects\/([A-Z0-9]+)\/tasks/)
            ? result.match(/projects\/([A-Z0-9]+)\/tasks/)[1]
            : result.match(/([A-Z0-9]+)/)
            ? result.match(/([A-Z0-9]+)/)[1]
            : result,
          name: ``,
          once: true,
          sound: true,
...
function watcherDraw(watcher) {
  ...
  name.className = `name`;
  name.textContent =
    watcher.name.length > 0
      ? watcher.name
      : watcher.project instanceof Object && typeof watcher.project.requester_name === `string`
      ? watcher.project.requester_name
      : watcher.id;
...
function watcherUpdate(watcher) {
  ...
  document.getElementById(id).getElementsByClassName(`name`)[0].textContent =
    watcher.name.length > 0
      ? watcher.name
      : watcher.project instanceof Object && typeof watcher.project.requester_name === `string`
      ? watcher.project.requester_name
  ...
  document.getElementById(id).getElementsByClassName(`stats`)[0].textContent = `Caught: ${
    caught > 0 ? caught : 0
  }; Searched: ${searched > 0 ? searched : 0}; PRE: ${pre > 0 ? pre : 0};`;
}
...
function watcherSettingsShow(watcher) {
  ...
  if (project instanceof Object) {
    ...
    document.getElementById(
      `watcher-settings-reward`,
    ).textContent = project.monetary_reward.amount_in_dollars.toMoneyString();
    document.getElementById(
      `watcher-settings-duration`,
    ).textContent = project.assignment_duration_in_seconds.toTimeString();
    document.getElementById(`watcher-settings-requirements`).textContent =
      (project.hit_requirements || project.project_requirements)
        .map((o) =>
...
async function catcherRun(forcedId) {
  ...
  if (catcher.paused.status === false && catcher.ids.length > 0) {
    const id =
      typeof forcedId === `string` && catcher.ids.includes(forcedId) === true
        ? forcedId
        : catcher.ids[(catcher.index = catcher.index >= catcher.ids.length - 1 ? 0 : catcher.index + 1)];
    const watcher = storage.watchers[id];

    var err;
    const response = await fetch(`https://workersandbox.mturk.com/projects/${id}/tasks/accept_random?format=json`, {
      credentials: `include`,
    }).catch((e) => (err = e));
      ...
      if (response.ok && response.url.indexOf(`tasks`) !== -1) {
        if (!watcher.project) {
          ...
          const json = JSON.parse(
            doc.querySelector(`[data-react-class="require('reactComponents/common/ShowModal')['default']"]`).dataset
              .reactProps,
          );

          watcher.project = watcher.project
            ? watcher.project
            : {
                requester_name: json.modalOptions.requesterName,
                requester_id: json.modalOptions.contactRequesterUrl.match(/requester_id.+?=([0-9A-Z]+)/)
                  ? json.modalOptions.contactRequesterUrl.match(/requester_id.+?=([0-9A-Z]+)/)[1]
                  : null,
                title: json.modalOptions.projectTitle,
                hit_set_id: response.url.match(/projects\/([A-Z0-9]+)\/tasks/)
                  ? response.url.match(/projects\/([A-Z0-9]+)\/tasks/)[1]
                  : null,
                monetary_reward: {
                ...
                project_requirements: [
                  { qualification_type: { name: `Unknown` }, comparator: `Unknown`, qualification_values: [``] },
                ],
              };
        }
...
function catcherCaptchaFound() {
  ...
  if (captcha.popup === true && typeof hit_set_id === `string` && hit_set_id.length > 10) {
    window.open(
      `https://workersandbox.mturk.com/projects/${hit_set_id}/tasks?ref=w_pl_prvw`,
      `hcPopup`,
